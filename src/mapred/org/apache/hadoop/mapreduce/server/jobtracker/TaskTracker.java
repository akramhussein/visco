...
public class TaskTracker {
	 ...
	 * @return the current <code>TaskTrackerStatus</code> of the <code>TaskTracker</code>
	 */
	...
	public void reserveSlots(TaskType taskType, JobInProgress job, int numSlots) {
		...
		if (taskType == TaskType.MAP) {
			if (jobForFallowMapSlot != null && !jobForFallowMapSlot.getJobID().equals(jobId)) {
				throw new RuntimeException(trackerName + 
						" already has slots reserved for "+ jobForFallowMapSlot +
						"; being asked to reserve "+ numSlots +" for "+ jobId);
			}
			jobForFallowMapSlot = job;
		} else if (taskType == TaskType.REDUCE) {
			if (jobForFallowReduceSlot != null && !jobForFallowReduceSlot.getJobID().equals(jobId)) {
				throw new RuntimeException(trackerName +
						" already has slots reserved for " + jobForFallowReduceSlot +
						"; being asked to reserve "+ numSlots +" for "+ jobId);
			}
