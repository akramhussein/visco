...
public class ReduceContext<KEYIN,VALUEIN,KEYOUT,VALUEOUT> extends TaskInputOutputContext<KEYIN,VALUEIN,KEYOUT,VALUEOUT> {
	...
	// custom constructor for the visco root reduce task
	// it does not contain the input. The functionality of this context is reduced.
	public ReduceContext (Configuration conf, TaskAttemptID taskid,
			RecordWriter<KEYOUT,VALUEOUT> output,
			Counter inputKeyCounter,
			Counter inputValueCounter,
	...
	public boolean nextKeyValue() throws IOException, InterruptedException {
		...
		if (hasMore) {
			next = input.getKey();
			nextKeyIsSame = comparator.compare(currentRawKey.getBytes(), 0, currentRawKey.getLength(), next.getData(),
